...
#define PAF_GPU_VULKAN   (1 << 2)

// Call once at startup (or rely on lazy init).
// Returns bitmask of available GPU backends (PAF_GPU_CUDA | PAF_GPU_DSTORAGE | PAF_GPU_VULKAN).
...
// Returns non-zero if Vulkan compute is available (libvulkan.so.1 / vulkan-1.dll + paf_sha256.spv).
...
typedef int (*paf_cuda_hash_flat_fn)(const uint8_t* host_buf, const uint64_t* offsets,
                                     ...
                                     uint8_t* out_hashes);
typedef int (*paf_vulkan_hash_flat_fn)(const uint8_t* host_buf, const uint64_t* offsets,
                                       const uint64_t* sizes, uint32_t count,
                                       uint8_t* out_hashes);
...
// Populated by paf_gpu_init() when Vulkan compute is available; NULL otherwise.
extern paf_vulkan_hash_flat_fn g_paf_vulkan_hash_flat;
