...
__global__ void paf_sha256_kernel(const uint8_t* data, const uint64_t* offsets, const uint64_t* sizes, uint8_t* hashes, uint32_t count) {
    ...
    // Final padding (Simplified for kernel example, real impl needs full SHA-256 padding logic)
    // ... (omitted for brevity, but would be implemented in full version)
    
    // Write out results
    #pragma unroll
    for (int i = 0; i < 4; i++) {
        hashes[idx * 32 + i] = (state[0] >> (24 - i * 8)) & 0xff;
        // ... (truncated)
    }
}

__global__ void paf_search_kernel(const char* path_buffer, const uint64_t* offsets, const uint32_t* lengths, const char* pattern, uint32_t pattern_len, uint32_t* results, uint32_t* match_count, uint32_t count) {
    uint32_t idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count) return;

    const char* path = path_buffer + offsets[idx];
    uint32_t len = lengths[idx];

    // Simple substring search (can be optimized with KMP/Boyer-Moore on GPU)
    bool found = false;
    if (len >= pattern_len) {
